...
void newlines_class_colon_pos(E_Token tok)
{
   ...
   else // tok == E_Token::CT_CONSTR_COLON
   {
   ...
   for (Chunk *pc = Chunk::GetHead(); pc->IsNotNullChunk(); pc = pc->GetNextNcNnl())
   {
      ...
      if (pc->Is(tok))
      {
         ...
         if (pc->Is(E_Token::CT_CONSTR_COLON))
         {
            LOG_FMT(LBLANKD, "%s(%d): pc orig line is %zu, orig col is %zu, text '%s', type is %s\n",
                    __func__, __LINE__, pc->GetOrigLine(), pc->GetOrigCol(), pc->GetLogText(), get_token_name(pc->GetType()));
            Chunk *paren_vor_value = pc->GetNextType(E_Token::CT_FPAREN_OPEN, pc->GetLevel());
