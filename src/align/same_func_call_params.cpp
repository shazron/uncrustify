      ...
      if (add_str != nullptr)
      {
         ...
         for (size_t idx = 0; idx < chunks.size(); idx++)
         {
            ...
            if (idx >= array_of_AlignStack.size())
            {
               ...
               {
                  if (  chunks[idx]->Is(E_Token::CT_NUMBER_FP)
                     || chunks[idx]->Is(E_Token::CT_NUMBER)
                     || chunks[idx]->Is(E_Token::CT_POS)
                     || chunks[idx]->Is(E_Token::CT_NEG))
                  {
...
void align_params(Chunk *start, std::deque<Chunk *> &chunks)
{
   ...
   bool  hit_comma = true;
   Chunk *pc       = start->GetNextType(E_Token::CT_FPAREN_OPEN, start->GetLevel());
